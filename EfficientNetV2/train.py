...
def parse_args():
    ...
    parser.add_argument('--weights', type=str,
                        default=r".\pretrained_model\pre_efficientnetv2-s.pth",
                        help='initial weights path')
    parser.add_argument('--save-path', type=str, default="../outputs/model")
    # training parameters
...
def main(args):
    ...
    train_images_path, train_images_label, val_images_path, val_images_label = read_split_data(args.dataset_path, args.save_path, args.val_ratio)
    ...
    # loading pretained parameters (if exists)
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > acc_best:
            print("epoch number of training: {0}, acc = {1}".format(epoch, val_acc))
            acc_best = val_acc

        torch.save(model.state_dict(), f"{args.save_path}-{epoch}.pth")
